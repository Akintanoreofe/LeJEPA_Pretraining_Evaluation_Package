...
# =========================================================
# LeJEPA — EfficientNet-B0 (Tile-Based)
# PAPER-MATCHED LeJEPA LOSS:
#   LeJEPA = (1 - λ) * L_pred + λ * SIGReg
# Geometric augmentations ONLY (no color/texture changes).
#
# Saves (locally under CHECKPOINT_DIR/interactive_plots):
...
#   - 1 CSV history file:
#       loss_history_efficientnetb0.csv
# =========================================================
...
BATCH_SIZE = 8
EPOCHS = 120
V = 2  # number of views per sample
...
PCA_EPOCHS = {0, EPOCHS // 2, EPOCHS - 1}  # epoch1, mid, last
...
class SIGReg(nn.Module):
    ...
    def forward(self, proj):
        # proj: (V, B, D) or (B, D); normalize to (V, B, D)
        if proj.dim() == 2:
...
# =========================================================
# DATASET (Tile-Based) — geometric-only augmentations
# =========================================================
class ForageTileDataset(Dataset):
    def __init__(self, root):
        root = Path(root)
...
class EfficientNetB0Encoder(nn.Module):
    def __init__(self, proj_dim=128):
        ...
        # EfficientNet-B0 feature dim = 1280 (same as MobileNetV2)
        feat_dim = eff.classifier[1].in_features  # robust way to get 1280
...
def lejepa_prediction_loss(proj: torch.Tensor) -> torch.Tensor:
    ...
    dif = mu - proj                       # (B,V,D)
    return dif.square().mean()            # scalar


# =========================================================
# VISUALS — Interactive PCA (HTML)
# =========================================================
@torch.no_grad()
def save_interactive_pca(net, loader, epoch, out_dir: Path, max_batches=60):
    ...
    for vs in loader:
        ...
        emb, _ = net(vs)    # (B,V,feat_dim)
        emb = emb.reshape(-1, emb.shape[-1])  # (B*V, feat_dim)
    ...
    fig = px.scatter_3d(
        x=Z[:, 0], y=Z[:, 1], z=Z[:, 2],
        opacity=0.7,
        title=f"EfficientNet-B0 Tile Embeddings — Epoch {epoch + 1}",
    )
...
def save_training_curves(history, out_dir: Path):
    ...
    fig.update_layout(
        title="Training Loss Curves — EfficientNet-B0",
        xaxis_title="Epoch",
        yaxis_title="Loss",
        hovermode="x unified",
...
# =========================================================
# SAVE LOSSES (CSV) — for later model comparison
# =========================================================
...
def main():
    ...
    plots_dir = ckpt_dir / "Eff_interactive_plots" / "efficientnetb0"
    ...
    net = EfficientNetB0Encoder(PROJ_DIM).to(DEVICE)
    sigreg = SIGReg().to(DEVICE)
    ...
    ckpt_path = ckpt_dir / "lejepa_efficientnetb0_tile_L2pred.pth"
    torch.save(net.state_dict(), ckpt_path)
    print(f"🖤 Model saved to {ckpt_path}")

    # Save visuals + history CSV
    save_training_curves(history, plots_dir)
    save_loss_history(history, plots_dir, model_name="efficientnetb0")
